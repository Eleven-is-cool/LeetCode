...
public class Main {
    ...
    public static Integer back(int[] values, int[] days, int N, int X){
        int[][] dp = new int[N+1][X+1];
        for(int n=1;n<N+1;n++){
            for(int x=1;x<X+1;x++){
                if(x-days[n-1]>=0){
                    dp[n][x] = Math.max(dp[n-1][x],dp[n-1][x-days[n-1]]+values[n-1]);
                }else {
                    dp[n][x] = dp[n-1][x];
                }
            }
    ...
    public static void main(String[] args) {
        ...
        while (sc.hasNext()) {
            ...
            System.out.println(back(values, days, N, X));
        }
    }
}
